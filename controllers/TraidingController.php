 ...
 */
class TraidingController extends MainController
{
    public function actionIndex()
    {
        return $this->render('index', [
            'state' => Traider::getState(),
    ...
    {
        return \json_encode(Traider::getState(), JSON_FORCE_OBJECT);
    }
		...
		if (!empty($state)) {
			...
			try {
				...
				if (!empty($payout)) {
					...
					$sms->send('8'.$user->phone, $msg.(number_format(abs($payout->sum), 2, '.', ' '))." RUB\nSOTACARD", \Yii::$app->params['sms_sender']);
				}
